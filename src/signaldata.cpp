...
public:
    ...
    inline void clear(void)
    {
        ...
        //reset boundingrect, left, top, width and height
        QPointF tl(1.0,1.0);
        boundingRect.setWidth(-2.0);
        boundingRect.setHeight(-2.0);
        boundingRect.setTopLeft(tl);
    }
    ...
    inline void append(const QPointF &sample)
    {
        ...
        //if bounding rect is still untouched since object creation
...
{
    //Deletes vector, re-reserves space
    //appends all values with x greater than limit
    d_data->lock.lockForWrite();
    ...
    //result is index of the first number left of limit
...
SignalData &SignalData::instance(int i)
{/*
...
*/
   ...
   //qDebug() << "Channel " << i;
    ...
    static SignalData valueVector[10];
